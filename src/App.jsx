...
export default function App() {
  ...
  // Load data + check URL params (from QR scan)
  useEffect(() => {
    const d = loadData();
    setData(d);
    setLoaded(true);
    ...
    if (boxId && d.boxes) {
      const found = d.boxes.find((b) => b.id === boxId);
  ...
  const addBox = (box) => {
    setData((p) => ({
      boxes: [...p.boxes, { ...box, id: Date.now().toString(), items: [], color: BOX_COLORS[p.boxes.length % BOX_COLORS.length] }]
    }));
    showToast('箱子已新增！');
    setView('home');
  };
  ...
  const deleteBox = (id) => {
    setData((p) => ({ boxes: p.boxes.filter((b) => b.id !== id) }));
    showToast('箱子已刪除', 'warn');
    setView('home');
  };

  const addItem = (boxId, item) => {
    setData((p) => ({
      boxes: p.boxes.map((b) =>
        b.id === boxId ? { ...b, items: [...b.items, { ...item, id: Date.now().toString() }] } : b
      )
    }));
    showToast('物品已新增！');
  };
  ...
  const updateBox = (boxId, updates) => {
    setData((p) => ({
      boxes: p.boxes.map((b) => (b.id === boxId ? { ...b, ...updates } : b))
    }));
  };
  ...
  const handleQRResult = useCallback((value) => {
    ...
    const found = data.boxes.find((b) => b.id === value);
    if (found) { openBox(found); showToast(`已找到：${found.name}`); return; }
    ...
    <div className="app">
      ...
      <header className="header">
        ...
        <div style={{ flex: 1 }}>
          ...
          {view === 'home' && <span className="subtitle">{data.boxes.length} 個箱子</span>}
        </div>
      </header>
            ...
            <div>
              ...
              {searchResults.map(({ box, matchItems }) => (
                <div key={box.id} className="list-card" onClick={() => openBox(box)}>
                  <div className="color-dot" style={{ background: box.color }} />
                  <div style={{ flex: 1 }}>
                    ...
                    {matchItems.length > 0 && (
                      <div className="match-items">
                        {matchItems.map((i) => (
                          <span key={i.id} className="match-chip">{i.photo && <Icons.Image />} {i.name}</span>
                        ))}
                      </div>
                    )}
                  </div>
            ...
            <div className="box-list">
              {data.boxes.map((box) => (
                <div key={box.id} className="box-card" onClick={() => openBox(box)}>
                  <div className="box-stripe" style={{ background: box.color }} />
                  <div className="box-body">
                    ...
                    <div className="box-preview">
                      ...
                      {box.items.slice(0, 3).map((i) =>
                        i.photo
                          ? <img key={i.id} src={i.photo} alt="" className="preview-thumb" />
      ...
      {/* Add Box */}
      {view === 'add' && <AddBoxView onAdd={addBox} onCancel={() => setView('home')} />}

      {/* Box Detail */}
      {view === 'box' && selectedBox && (
        <BoxDetailView
          box={data.boxes.find((b) => b.id === selectedBox.id) || selectedBox}
          baseUrl={baseUrl}
          onAddItem={(item) => addItem(selectedBox.id, item)}
          onDeleteItem={(itemId) => deleteItem(selectedBox.id, itemId)}
          onDeleteBox={() => deleteBox(selectedBox.id)}
          onUpdateBox={(u) => updateBox(selectedBox.id, u)}
...
// ─── Add Box ───
function AddBoxView({ onAdd, onCancel }) {
    ...
    <div className="content">
      <div className="card">
        ...
        <div className="form-actions">
          ...
          <button className="btn-primary" style={{ opacity: name.trim() ? 1 : 0.5 }} onClick={() => name.trim() && onAdd({ name: name.trim(), location: location.trim() })} disabled={!name.trim()}>
            確認新增
          </button>
        </div>
      </div>
...
// ─── Box Detail ───
function BoxDetailView({ box, baseUrl, onAddItem, onDeleteItem, onDeleteBox, onUpdateBox, onImageClick }) {
  ...
  const handleAdd = () => {
    ...
    onAddItem({ name: newItem.trim(), note: newNote.trim(), qty: parseInt(newQty) || 1, photo: newPhoto });
    setNewItem(''); setNewNote(''); setNewQty('1'); setNewPhoto(null);
    ref.current?.focus();
  };

  const hasPhotos = box.items.some((i) => i.photo);

  return (
    <div className="content">
      {/* Box info */}
      <div className="card box-info" style={{ borderLeft: `4px solid ${box.color}` }}>
        <div className="box-info-top">
          <div>
            <div className="box-info-name">{box.name}</div>
            <div className="box-info-loc">{box.location || '未設定位置'}</div>
          </div>
          <span className="badge lg">{box.items.length} 項</span>
        </div>
      ...
      {!showAdd ? (
        ...
        <div className="card add-item-card">
          ...
          <div className="add-item-actions">
            ...
            <button className="btn-small-primary" style={{ opacity: newItem.trim() ? 1 : 0.5 }} onClick={handleAdd} disabled={!newItem.trim()}>新增</button>
          </div>
        </div>
      )}

      {/* View toggle */}
      {box.items.length > 0 && hasPhotos && (
        ...
        <div className="item-list">
          {box.items.map((item, idx) => (
            <div key={item.id} className="item-row">
              {item.photo ? (
                <img src={item.photo} alt="" className="item-thumb" onClick={() => onImageClick(item.photo)} />
              ) : (
        ...
        <div className="photo-grid">
          {box.items.map((item) => (
            <div key={item.id} className="photo-grid-item">
              {item.photo ? (
                <img src={item.photo} alt="" className="photo-grid-img" onClick={() => onImageClick(item.photo)} />
              ) : (
      ...
      <div className="danger-zone">
        {!confirmDel ? (
          ...
          <div className="confirm-row">
            ...
            <button className="danger-confirm" onClick={onDeleteBox}>確認刪除</button>
